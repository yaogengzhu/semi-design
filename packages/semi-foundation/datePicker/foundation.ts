...
import { Type, DateInputFoundationProps, InlineInputValue } from './inputFoundation';
...
export interface DatePickerAdapter extends DefaultAdapter<DatePickerFoundationProps, DatePickerFoundationState> {
    ...
    updateInlineInputValue: (inlineInputValue: InlineInputValue) => void;
    setInlineInputFocus: () => void;
...
export default class DatePickerFoundation extends BaseFoundation<DatePickerAdapter> {
    ...
    /**
     * clear inline input value when close panel
     */
    clearInlineInputValue() {
        const { inlineInput } = this._adapter.getProps();
        if (inlineInput) {
            this._adapter.updateInlineInputValue(null);
        }
    }
    ...
    /**
     * inline input 变化时需要更新以下 state 状态
     *  - inputInlineValue（总是）
     *  - inputValue（可以解析为合法日期时）
     *  - value（可以解析为合法日期时）
     */
    handleInlineInputChange(options: { inlineInput: string, format: string, inlineInputValue: InlineInputValue }) {
        const { inlineInput, format, inlineInputValue } = options;
        const _isMultiple = this._isMultiple();
        const result = _isMultiple ? this.parseMultipleInput(inlineInput, format) : this.parseInput(inlineInput, format);
        const { value: stateValue } = this.getStates();
        ...
        this._adapter.updateInlineInputValue(inlineInputValue);
    }
    ...
    handleSelectedChange(value: Date[], options?: { fromPreset?: boolean; needCheckFocusRecord?: boolean }) {
        const { type, format, rangeSeparator, inlineInput } = this._adapter.getProps();
        const { value: stateValue } = this.getStates();
        ...
        let inputValue, inlineInputValue;
        if (!this._someDateDisabled(changedDates)) {
            inputValue = this._isMultiple() ? this.formatMultipleDates(dates) : this.formatDates(dates);
            if (inlineInput) {
                const inlineInputFormatToken = getInlineInputFormatToken({ format, type });
                const inlineInput = this._isMultiple() ? this.formatMultipleDates(dates, undefined, inlineInputFormatToken) : this.formatDates(dates, inlineInputFormatToken);
                inlineInputValue = getInlineInputValueFromInlineInput({ inputValue: inlineInput, type, rangeSeparator  });
            }
    ...
    handlePanelVisibleChange(visible: boolean) {
        if (visible) {
            this._adapter.setInlineInputFocus();
            /**
